...
async function writeJson (sheet, variablePath, dist) {
  ...
  await fs.writeFile(dist, prettier.format(`export default ${JSON.stringify(lessVars, null, 2)}`, { semi: false, singleQuote: true, parser: 'babel' }))
}
...
async function writeDesc (src, dist) {
  ...
  await fs.writeFile(dist, prettier.format(`export default ${JSON.stringify(desc, null, 2)}`, { semi: false, singleQuote: true, parser: 'babel' }))
}
